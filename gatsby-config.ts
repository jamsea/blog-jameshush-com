        ...
        feeds: [
          {
            serialize: ({
              ...
              return allMarkdownRemark.nodes.map((node: Node) => {
    ...
    },
    {
      ...
      options: {
        ...
        resolvePages: ({
          ...
          const pages = allPosts.map(
            (post: { fields: { slug: any }; frontmatter: { date: any } }) => {
              const path = post.fields.slug
              const page = allPages.find((p: { path: any }) => p.path === path)
              if (page) {
